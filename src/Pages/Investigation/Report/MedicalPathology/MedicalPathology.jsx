...
const MedicalPathology = () => {
  const navigate = useNavigate();
  const { state } = useLocation();
  // console.log(state)
  const patientData = state?.record;
  // console.log(patientData.package.investigation_id)
  ...
  // const [statusId, setStausId] = useState(2);
  ...
  const getPathoComment = () => {
    axios
      .get(
        `${process.env.REACT_APP_API_BASE_URL}/reports/get-comments/${patientData?.user_id}/2}`
      )
      .then((response) => {
  ...
  const getActiveInvTests = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setActiveInvTests1(data?.actInvTestP1);
        setActiveInvTests2(data?.actInvTestP2);
        // console.log(data);
      })
      .catch((err) => console.log(err));
  };
    ...
    <>
      <div className="card p-3 medical-report">
        <div className="row mb-4">
          <h6>
            Medical Pathology Report for {patientData?.user.f_name}{" "}
...
export default MedicalPathology;
